<script>
	...
	import BigNeedLoginButton from '$lib/components/BigNeedLoginButton.svelte';
	...
	onMount(() => {
		if (!$isLoggedIn) isLoginPopupShown.set(true);
	});
</script>
	...
	<div style="--book-qna-primary: #5bb3ff; --book-qna-secondary: #ff0606">
		<header class="tw-flex tw-items-center tw-flex-col tw-my-16">
			<div class="tw-font-heading tw-text-4xl tw-relative tw-mb-4">
				안녕하세요, 박효신님
				<div
					class="tw-absolute -tw-left-4 -tw-right-4 tw-h-3 tw-bottom-1 -tw-z-10 tw-bg-[#d6efff]"
				/>
			</div>
